...
def update_module_name(old_module_name, new_module_name):
    ...
    if old_module_name in data_dict:
        ...
        print(Fore.LIGHTGREEN_EX +
              f"instance {old_module_name} renamed to {new_module_name}." + Fore.RESET)
    else:
...
def update_port_name(old_port_name, new_port_name):
    with open(f"{Baseboard_path}/{Json_Top_file}.json", 'r') as json_file:
        ...
        if old_port_name in data_dict['clock']['ports']:
            data_dict['clock']['ports'][new_port_name] = data_dict['clock']['ports'].pop(
                old_port_name)
            with open(f"{Baseboard_path}/{Json_Top_file}.json", 'w') as outfile:
                json.dump(data_dict, outfile, indent=4)
            print(Fore.LIGHTGREEN_EX +
                  f"Successfully updated port name from {old_port_name} to {new_port_name}" + Fore.RESET)
        else:
            print(Fore.LIGHTGREEN_EX +
                  f"Port with name {old_port_name} not found in file" + Fore.RESET)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file_name', type=str,
                        help='name of file where instance or port need to be renamed')
    parser.add_argument('-u', '--update_name', type=str,
                        help='name of file where instance or port need to be renamed')

    parser.add_argument('-p', '--port', type=str,
                        help='name of port need to be renamed')
    parser.add_argument('-i', '--instance', type=str,
                        help='name of instance need to be renamed')

    args = parser.parse_args()
    Top_level_file = args.file_name
    current_name = args.update_name
    LAGO_USR_INFO()
    ...
    if args.port:
        port = args.port
        rename_port(Top_level_file, port, current_name)
        update_port_name(port, current_name)
    if args.instance:
        instance = args.instance
        update_module_name(instance, current_name)
        rename_module(Top_level_file, instance, current_name)
