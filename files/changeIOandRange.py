        ...
        for i, line in enumerate(lines):
            if re.search(fr"\[(\d.*:\d.*)\].*\b{port_name}\b", line):
                ...
                if old_range == new_range:
                    print(
                        Fore.RED + f"Error: Port status is already {new_range}" + Fore.RESET)
                    Success = False
                    return
                else:
                    updated_line = re.sub(
                        fr"\[(\d.*:\d.*)\]", f"{new_range}", line)
                    lines[i] = updated_line
                    with open(file_name, "w") as f_out:
                        f_out.writelines(lines)
                    print(
                        Fore.LIGHTGREEN_EX + f"Range for {port_name} port updated from {old_range} to {new_range}" + Fore.RESET)
                    Success = True
                    return Success
        if not found:
            print(
                Fore.RED + f"Error: {port_name} or range not found in {file_name}." + Fore.RESET)
            Success = False
            return Success
...
def change_IO_status(file_name, new_status, port_name):
    with open(file_name, "r") as f:
        ...
        for i, line in enumerate(lines):
            if re.search(fr'\b(input|output)\b.*\b{port_name}\b', line):
                ...
                if port_type == new_status:
                    print(
                        Fore.RED + f"Error: Port status is already {new_status}" + Fore.RESET)
                    Success = False
                    return Success
                else:
        ...
        if not found:
            print(
                Fore.RED + f"Error: {port_name} not found in {file_name}." + Fore.RESET)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o', '--operation', choices=['range', 'update_range_json', 'stat', 'change_IO_status_json'])
    parser.add_argument(
        '-nr', '--new_range', help='New range to update (for update_range and update_range_json operations)')
    parser.add_argument(
        '-p', '--port_name', help='Name of the port to update (for update_range, update_range_json, change_IO_status, and change_IO_status_json operations)')
    parser.add_argument('-ns', '--new_status', choices=[
                        'input', 'output'], help='New status to update (for change_IO_status and change_IO_status_json operations)')
    ...
    if args.operation == 'stat':
        Success = change_IO_status(
            Top_level_file, args.new_status, args.port_name)
        if Success:
